...
setMethod(f = "reset", signature = "EnrichSNP", definition = function(object, i) {
  switch(EXPR = i,
    ...
    "Table" = {
      object@Table <- matrix(0, ncol = 2, nrow = 2)
    },
    "EnrichmentRatio" = {
      object@EnrichmentRatio <- numeric()
    },
    "Z" = {
      object@Z <- numeric()
    },
    "PValue" = {
      object@PValue <- numeric()
    },
    "Resampling" = {
      object@Resampling <- matrix(0, ncol = 5, nrow = 0)
    },
