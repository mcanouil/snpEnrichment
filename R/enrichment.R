...
setMethod(f = "print", signature = "Enrichment", definition = function(x, what = "Genome", type = c("eSNP", "xSNP")) {
  ...
  if (length(what) == 1) {
    ...
    resTmp <- lapply(what, function(iWhat) {
      print(x["Chromosomes", iWhat], type = type)
    })
  ...
  if (missing(j)) {
    ...
    if (max(j) > nbChr) {
      ...
      if (length(j) > 1) {
        switch(EXPR = i,
          "Signal" = {
            return(eval(parse(text = paste0("list(", paste(paste0("x@Chromosomes$Chrom", j, "@Data[, c('SNP', 'PVALUE')]"), collapse = ", "), ")"))))
          },
          "Data" = {
        ...
        switch(EXPR = i,
          "Signal" = {
            return(eval(parse(text = paste0("x@Chromosomes$Chrom", j, "@Data[, c('SNP', 'PVALUE')]"))))
          },
          "Data" = {
            return(eval(parse(text = paste0("x@Chromosomes$Chrom", j, "@Data"))))
          },
          "LD" = {
            return(eval(parse(text = paste0("x@Chromosomes$Chrom", j, "@LD"))))
          },
          "Call" = {
            return(x@Call)
          },
...
setMethod(f = "[<-", signature = "Enrichment", definition = function(x, i, j, value) {
  ...
  if (missing(j)) {
    switch(EXPR = i,
      ...
      "Data" = {
        stop('[Enrichment:set] "Data" is not available for Set function.', call. = FALSE)
      },
      "LD" = {
        stop('[Enrichment:set] "LD" is not available for Set function.', call. = FALSE)
      },
      "Call" = {
        x@Call <- value
      },
      ...
      "List" = {
        stop('[Enrichment:set] "List" is not available for Set function.', call. = FALSE)
      },
      "Table" = {
        stop('[Enrichment:set] "Table" is not available for Set function.', call. = FALSE)
      },
      "EnrichmentRatio" = {
        stop('[Enrichment:set] "EnrichmentRatio" is not available for Set function.', call. = FALSE)
      },
      "Z" = {
        stop('[Enrichment:set] "Z" is not available for Set function.', call. = FALSE)
      },
      "PValue" = {
        stop('[Enrichment:set] "PValue" is not available for Set function.', call. = FALSE)
      },
      "Resampling" = {
        stop('[Enrichment:set] "Resampling" is not available for Set function.', call. = FALSE)
      },
      "Chromosomes" = {
        x@Chromosomes <- value
      },
      "Stats" = {
        stop('[Enrichment:set] "Stats" is not available for Set function.', call. = FALSE)
      },
    ...
    if (max(j) > nbChr) {
      ...
      if (length(j) > 1) {
        ...
        switch(EXPR = i,
          ...
          "eSNP" = {
            x@Chromosomes[[j]]@eSNP <- value
          },
          "xSNP" = {
            x@Chromosomes[[j]]@xSNP <- value
          },
          "List" = {
            stop('[Enrichment:set] "List" is not available for Set function.', call. = FALSE)
          },
          ...
          "Chromosomes" = {
            x@Chromosomes[[j]] <- value
          },
          "Stats" = {
            stop('[Enrichment:set] "Stats" is not available for Set functions.', call. = FALSE)
          },
          stop("[Enrichment:set] ", i, ' is not a "Enrichment" slot.', call. = FALSE)
        )
      }
...
setMethod(f = "reSample", signature = "Enrichment", definition = function(object, nSample = 100, empiricPvalue = TRUE, MAFpool = c(0.05, 0.10, 0.2, 0.3, 0.4, 0.5), mc.cores = 1, onlyGenome = TRUE) {
  if (!missing(object)) {
    ...
    if (onlyGenome == FALSE) {
      ...
      for (iChr in seq_len(22)) {
        cat("  Chromosome ", if (nchar(iChr) == 1) {
          paste0("0", iChr)
        } else {
          iChr
        }, ": ", sep = "")
